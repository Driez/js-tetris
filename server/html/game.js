...
class TetrisGame {
	...
	triggerGameOver(){
		...
		this.setName();
		this.uploadScore();
		this.io.displayHighscores(this.getHighscores());
	}

setName(){
	this.name = window.prompt("Please enter your Name", "Player");
	}

	uploadScore(){
		return $.post("http://localhost:8080", {name: this.username ,score: this.score}, ()=>{
			console.log(data);
			return data;
		});
	}
	...
	getHighscores(){
		return $.get("http://localhost:8080", data =>{
			console.log(data);
			return data;
		});
	}
